...
redhawk_image="/home/test/images/bzImage-redhawk-virtual"
rootfs_path="/home/test/images/rootfs-ssh-pci.cpio"
...
def jailhouse_cell_linux(config_path,console_path):
	proc=subprocess.Popen(['jailhouse cell linux %s %s -i %s -c "console=%s"' % (config_path,redhawk_image,rootfs_path,console_path)], stdout=subprocess.PIPE,shell=True)
	stdout,stderr = proc.communicate()
	rc = proc.poll()
	if rc != 0:
